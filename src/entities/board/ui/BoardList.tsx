...
interface BoardListProps {
  /** @param {PostInfo[]} posts - 전체 게시글 정보 */
  posts: PostInfo[];
  /** @param {number} totalCount -  전체 게시물 수 */
  totalCount: number;
    ...
    <div className="flex flex-col gap-y-2">
      <div className="flex justify-between gap-2 border-b border-gray-200 px-2 pb-3 text-center text-sm text-gray-500">
        <div className="flex gap-2">
          {showNumber && <span className="w-12">번호</span>}
          <span className="text-left">게시글 제목</span>
        </div>

        <div className="flex gap-2">
          <span className="w-[6.25rem]">닉네임</span>
          <span className="w-12">조회수</span>
          <span className="w-12">추천</span>
          <span className="w-12">일자</span>
        </div>
      </div>
      {/* 공지사항 부분 주석처리 */}
